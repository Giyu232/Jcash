...
public class Database {
    ...
    private String url = "jdbc:mysql://localhost:3306/regencia_db";
    ...
    public List<TransactionMdl> getTransactionsByUserID(int userId) throws SQLException {
        List<TransactionMdl> trans = new ArrayList<TransactionMdl>();
        PreparedStatement ps = this.connection.prepareStatement("Select * from transaction WHERE `user_id` = ?");
        ps.setInt(1, userId);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            trans.add(new TransactionMdl(rs.getString("transaction_type"), rs.getDouble("amount"), rs.getString("details"), rs.getDate("date"), rs.getInt("user_id")));
        }
        return trans;
    }

    public TransactionMdl addTransactionByUserId(String type, double amount, String details, Date date, int userId) throws SQLException {
        ...
        int rs = ps.executeUpdate();
        if (rs > 0) {
            return new TransactionMdl(type, amount, details, date, userId);
        }
        return null;
    }
